...
from LEGEND.info import (db, add, is_served_call, add_active_video_chat, add_served_call, add_active_chat, gen_thumb, download, remove_active, joinch)
from LEGEND.Data import (get_logger, get_userbot, get_call, get_logger_mode, get_group, get_channel)
import asyncio
...
async def aii(client: Client, message):
   ...
   try:
    ...
    raw_list = []
    async for msg in user.get_chat_history("L109N"):
        if msg.audio:
          raw_list.append(msg)
    ...
    chat_id = message.chat.id
    user_id = message.from_user.id if message.from_user else "UR_ML"
    videoid = None
    link = None
...
async def play(client: Client, message):
  ...
  if not message.reply_to_message:
     ...
     if await is_served_call(client, message.chat.id):
         ...
         requester = chatname if LEGEND.views else f"[{message.from_user.first_name}](tg://user?id={message.from_user.id})"
         if message.from_user:
          if message.from_user.photo:
           ...
           ahmed = await client.get_chat("UR_ML")
