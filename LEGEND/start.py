...
from LEGEND.info import (is_served_chat, add_served_chat, is_served_user, add_served_user, get_served_chats, get_served_users, del_served_chat, joinch)
from LEGEND.Data import (get_dev, get_bot_name, set_bot_name, get_logger, get_group, get_channel, get_dev_name, get_groupsr, get_channelsr, get_userbot)
...
from pyrogram.enums import ChatType, ChatMemberStatus, ParLEGENDde, ChatMemberStatus
import os
import re
import textwrap
import aiofiles
...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "UR_ML":
      try:
         ...
         await client.set_administrator_title(chat_id, user_id, "ليجند")
      except:
        pass
      return await message.reply_text(f"**♪ انضم المطو ليجند للشات  🕊 .\n♪ مرحبا بك : @L120N  🕊 .**")
    dev = await get_dev(bot_username)
...
async def UR_ML(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
@Client.on_message(filters.command("ليجند","ليجيند"))
...
"مره واحد اسمو ليجند  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
    ...
    await message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup(
                    [
                        ...
                        [
                            InlineKeyboardButton("@𝑳 𝑬 𝑮 𝑬 𝑵 𝑫.❤_ ", user_id=f"ahmedelnqyb")
                        ],
                        [
...
@Client.on_message(filters.command(["مبرمج السورس","ليجند","المبرمج"], ""))
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="UR_ML")
...
@Client.on_message(filters.command(["قناه المطور","vsvsv cc ","cvv ccxv"], ""))
async def sultan(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="L112N")
     name = user.first_name
     username = user.username 
     bio = user.bio
     user_id = user.id
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["UR_ML"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["UR_ML"]:
   ...
   if message.from_user.username == "UR_ML":
     await message.reply_text(f"**♪ تم تنفيذ الامر بواسطة المطور ليجند  🕊 .**")
   else:
...
async def tiillllli(client, message):
 if message.from_user.username in ["UR_ML"]:
   ...
   if message.from_user.username == "UR_ML":
     await message.reply_text(f"**♪ تم تنفيذ الامر بواسطة مبرمج السورس  🕊 .**")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @UU_LE2  🕊 .**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @UU_LE2  🕊 .**")
...
async def videoo(client, message):
  ...
  if len(listvid) == 0:
   user = await get_userbot(client.me.username)
   async for msg in user.get_chat_history("videi_LEGEND"):
      if msg.video:
        listvid.append(msg.id)
  id = random.choice(listvid)
  video = f"https://t.me/videi_LEGEND/{id}"
  await message.reply_video(video=video, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @UU_LE2  🕊 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["L120N"]:
         await message.reply_text("**♪ رتبتك هي : مبرمج السورس  🕊 .**")
         return
     if userr.username in ["L112N"]:
         await message.reply_text("**♪ رتبتك هي : مبرمج السورس 🕊 .**")
         return
     if userr.id == dev:
